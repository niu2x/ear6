...
通过 Homebrew 安装 Boost 和 OpenSSL，并构建 Release 版原生 macOS 宿主。该任务
关闭可选 Qt 宿主，运行核心与 Desktop common 测试，并检查 `Ear6.app` 没有动态
链接 Ear6 核心。通过后会上传保留 macOS bundle 元数据的 ZIP，作为保留 14 天的
Actions Artifact。
