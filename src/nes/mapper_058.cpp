...
void Mapper058::init(const RomInfo& info, const std::vector<uint8_t>& prg_rom, const std::vector<uint8_t>& chr_rom) {
...
void Mapper058::write_register(uint16_t addr, uint8_t value) {
    (void)value;
    uint8_t prg_bank = addr & 0x07;
    if (addr & 0x40) {
        select_prg_page(0, prg_bank);
        select_prg_page(1, prg_bank);
    } else {
        select_prg_page_2x(0, prg_bank & 0x06);
    }
