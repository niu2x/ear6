...
void Mapper061::init(const RomInfo& info, const std::vector<uint8_t>& prg_rom, const std::vector<uint8_t>& chr_rom) {
...
void Mapper061::write_register(uint16_t addr, uint8_t value) {
    (void)value;
    uint8_t prg_page = ((addr & 0x0F) << 1) | ((addr >> 5) & 0x01);
    if (addr & 0x10) {
