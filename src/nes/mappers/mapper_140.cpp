...
void Mapper140::init(const RomInfo& info, const std::vector<uint8_t>& prg_rom, const std::vector<uint8_t>& chr_rom) {
...
void Mapper140::write_register(uint16_t addr, uint8_t value) {
    (void)addr;
    select_prg_page(0, (value >> 4) & 0x03);
