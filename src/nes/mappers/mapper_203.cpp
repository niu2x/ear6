...
void Mapper203::init(const RomInfo& info, const std::vector<uint8_t>& prg_rom, const std::vector<uint8_t>& chr_rom) {
...
void Mapper203::write_register(uint16_t addr, uint8_t value) {
    (void)addr;
    select_prg_page(0, value >> 2);
    select_prg_page(1, value >> 2);
