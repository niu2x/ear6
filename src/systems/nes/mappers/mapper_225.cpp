...
void Mapper225::init(const RomInfo& info, const std::vector<uint8_t>& prg_rom, const std::vector<uint8_t>& chr_rom) {
...
void Mapper225::write_register(uint16_t addr, uint8_t value) {
    (void)value;
    uint8_t high_bit = (addr >> 8) & 0x40;
    uint8_t prg_page = ((addr >> 6) & 0x3F) | high_bit;
    if (addr & 0x1000) {
    ...
    select_chr_page(0, (addr & 0x3F) | high_bit);
    set_mirroring_type((addr & 0x2000) ? MirroringType::HORIZONTAL : MirroringType::VERTICAL);
}
