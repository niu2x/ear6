...
TEST(ChoplifterRegression, Frame30) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR) + "/tests/local-roms/nes/mapper_3/Choplifter (J).nes";
...
TEST(Smb3Regression, Frame60) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR) + "/tests/local-roms/nes/mapper_4/Super Mario Bros 3 (J).nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
struct Mapper19TestEntry {
    const char* filename;
    int frame;
    const char* expected_md5;
};

static const Mapper19TestEntry kMapper19Tests[] = {
    {"Battle Fleet (J).nes", 60, "a229f23e3cf90f45412519c14697a999"},
    {"Youkai Douchuuki (J).nes", 128, "fe98488c79ec5de6a8ac003e8d23534b"},
};

class Mapper19RegressionTest : public ::testing::TestWithParam<Mapper19TestEntry> {};

TEST_P(Mapper19RegressionTest, Frame) {
    const auto& param = GetParam();
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_19/" + param.filename;
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST_P(VrcRegressionTest, Frame) {
    const auto& param = GetParam();
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_" + std::to_string(param.mapper)
                           + "/" + param.filename;
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
...
TEST(Mapper64Regression, HeaderMapperProbeFrame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_64/Excitebike (JU).nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST(Mapper45Regression, BrainSeriesFrame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_45/BrainSeries13in1.nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
    ...
    ASSERT_NE(fb, nullptr);
    EXPECT_EQ(ppm_md5(fb, 256, 240),
              "cfbd88b698a3f390a351800a1a8918b8");
    ear6_destroy(ctx);
}

TEST(OldINesHeaderRegression, DiskDudeMapperBitsIgnored) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_4/SCON.nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST(Mapper68Regression, AfterBurnerFrame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_68/After Burner 2 (J).nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST(Mapper69Regression, BatmanFrame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_69/Batman (J).nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
    ...
    ASSERT_NE(fb, nullptr);
    EXPECT_EQ(ppm_md5(fb, 256, 240),
              "fe6d1cf605ea36d725a848feed7ebdc8");
    ear6_destroy(ctx);
}

TEST(Mapper74Regression, SuperRobotWars2Frame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_74/srw2.nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
    ...
    ASSERT_NE(fb, nullptr);
    EXPECT_EQ(ppm_md5(fb, 256, 240),
              "117c5862753a93eeaf2fc77f4247e8ee");
    ear6_destroy(ctx);
}

TEST(Mapper90Regression, FinalFantasy3Frame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_90/finalf3.nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
    ...
    ASSERT_NE(fb, nullptr);
    EXPECT_EQ(ppm_md5(fb, 256, 240),
              "bcdabff1a1a0d2e02db44117b0fe3406");
    ear6_destroy(ctx);
}

TEST(Mapper99Regression, VsBattleCityFrame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_99/vs battle city.nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST(Mapper118Regression, ArumajiroFrame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_118/Arumajiro (J).nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
    ...
    ASSERT_NE(fb, nullptr);
    EXPECT_EQ(ppm_md5(fb, 256, 240),
              "e81e270a0736ec54911f1d891e13d787");
    ear6_destroy(ctx);
}

TEST(Mapper245Regression, HeaderMapperProbeFrame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_245/Yong Ze Do Re Long 6 (C).nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST(Mapper252Regression, ThreeGoFrame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/tests/local-roms/nes/mapper_252/3GO.NES";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST_P(Mapper3RegressionTest, Frame) {
    const auto& param = GetParam();
    std::string rom_path = std::string(EAR6_SOURCE_DIR) + "/tests/local-roms/nes/mapper_3/" + param.filename;
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST_P(Mapper0RegressionTest, Frame) {
    const auto& param = GetParam();
    std::string rom_path = std::string(EAR6_SOURCE_DIR) + "/tests/local-roms/nes/mapper_0/" + param.filename;
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST_P(Mapper2RegressionTest, Frame) {
    const auto& param = GetParam();
    std::string rom_path = std::string(EAR6_SOURCE_DIR) + "/tests/local-roms/nes/mapper_2/" + param.filename;
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST_P(Mapper1RegressionTest, Frame) {
    const auto& param = GetParam();
    std::string rom_path = std::string(EAR6_SOURCE_DIR) + "/tests/local-roms/nes/mapper_1/" + param.filename;
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
