...
TEST(Ear6State, SupportedNesMappersContinueDeterministically) {
    const uint8_t mappers[] = {
        3, 6, 7, 8, 9, 10, 11, 13, 15, 17, 18, 32, 33, 34, 35, 38, 39,
        40, 41, 42, 43, 46, 50, 57, 58, 60, 61, 62, 64, 65, 66, 67, 68,
        70, 71, 72, 73, 75, 76, 77, 78, 79, 86, 87, 88, 89, 90, 92, 93,
        94, 95, 99, 101, 107, 112, 113, 117, 133, 140, 143, 144, 145,
        146, 148, 149, 150, 151, 154, 156, 170, 174, 180, 184, 185, 200,
        202, 203, 204, 206, 213, 214, 216, 221, 225, 226, 227, 229, 230,
...
TEST_P(Mapper19RegressionTest, Frame) {
    ...
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/assets/nes/rom/mapper_19/" + param.filename;
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST(Mapper45Regression, BrainSeriesFrame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/assets/nes/rom/mapper_45/BrainSeries13in1.nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST(Mapper69Regression, BatmanFrame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/assets/nes/rom/mapper_69/Batman (J).nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST(Mapper74Regression, SuperRobotWars2Frame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/assets/nes/rom/mapper_74/srw2.nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST(Mapper90Regression, FinalFantasy3Frame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/assets/nes/rom/mapper_90/finalf3.nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST(Mapper118Regression, ArumajiroFrame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/assets/nes/rom/mapper_118/Arumajiro (J).nes";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
...
TEST(Mapper252Regression, ThreeGoFrame256) {
    std::string rom_path = std::string(EAR6_SOURCE_DIR)
                           + "/assets/nes/rom/mapper_252/3GO.NES";
    FILE* rom_file = std::fopen(rom_path.c_str(), "rb");
    if (!rom_file) {
        GTEST_SKIP() << "Missing test ROM: " << rom_path;
    }
